...
prob = BifurcationProblem(F_chan, sol, par, (@lens _.α); recordFromSolution = (x,p) -> norminf(x))

optcont0 = ContinuationPar(dsmin = 0.01, dsmax = 0.05, ds= 0.01, pMax = 4.1, newtonOptions = NewtonPar(tol = 1e-9), maxSteps = 550)
br0 = @time continuation(prob, PALC(),optcont0, recordFromSolution = (x,p) -> norminf(x), normC = norminf, verbosity = 0)
plot(br0)

#################################################################################
# optanm = ContinuationPar(dsmin = 0.01, dsmax = 0.15, ds= 0.01, pMax = 4.1, newtonOptions = NewtonPar(tol = 1e-9))
#
...
#################################################################################
optanm = ContinuationPar(dsmin = 0.01, dsmax = 0.15, ds= 0.01, pMax = 4.1, newtonOptions = NewtonPar(tol = 1e1, verbose = false), detectBifurcation = 3, maxSteps = 6)
